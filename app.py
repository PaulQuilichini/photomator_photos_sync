...
from PySide6.QtGui import QDesktopServices, QFont, QPainter, QColor, QIcon, QPixmap
...
DEFAULT_SOURCE_FOLDER = Path("/Users/paulquilichini/Desktop/Photos - Local/2026/2026-04 Japan")
PHOTOS_SETTINGS_URL = "x-apple.systempreferences:com.apple.preference.security?Privacy_Photos"
...
HEADER_LOGO_SIZE_PX = round(CONTROL_HEIGHT_PX * 1.8)
...
def app_support_dir() -> Path:
    path = Path.home() / "Library" / "Application Support" / "PhotomatorFlagSync"
    path.mkdir(parents=True, exist_ok=True)
    return path


def first_existing_path(*candidates: str) -> Path:
...
APP_ICON_PATH = first_existing_path("logo_poru_main.svg", "logo_poru.svg", "pq_logo_smiling.png", "pq_logo_smiling.svg")
APP_HEADER_LOGO_PATH = first_existing_path("logo_poru_main.svg", "logo_poru.svg", "pq_logo_smiling.svg", "pq_logo_smiling.png")
...
def parse_xmp_flag(sidecar_path: Path) -> bool:
    ...
    for element in tree.iter():
        ...
        for key, value in element.attrib.items():
            if local_name(key) in PICK_NAMES and value.strip() == "1":
                return True
    return False
...
def extract_embedded_flagged(paths: list[Path], progress=None) -> list[Candidate]:
...
class ScanWorker(QObject):
    ...
    def __init__(self, folder: str, recursive: bool, photos_bridge: PhotosLibraryBridge) -> None:
        super().__init__()
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Photomator Flag Sync")
        self.resize(980, 680)
        ...
        for button in (self.browse_button, self.scan_button, self.import_button, self.dupe_scan_button, self.dupe_delete_button):
    ...
    def start_scan(self) -> None:
        ...
        self.scan_worker = ScanWorker(str(folder), self.recursive_checkbox.isChecked(), self.photos_bridge)
    ...
    def scan_finished(self, candidates: list[Candidate], error_message: str) -> None:
        ...
        if error_message:
            ...
            self.show_photos_access_dialog(error_message)
            self.set_busy(False)
            return
    ...
    def dupe_scan_finished(self, duplicates: list[DuplicateGroup], error_message: str) -> None:
        ...
        if error_message:
            self.dupe_summary_label.setText(error_message)
            self.append_log(error_message)
            self.show_photos_access_dialog(error_message)
            self.set_busy(False)
            return
